...
export default function Resources() {
  return (
    <main className="min-h-screen bg-[#f5fbff] text-slate-900">
      ...
      <section className="overflow-hidden bg-gradient-to-br from-[#0077c8] via-[#1887d4] to-[#008c5a] px-10 py-20">
        <div className="page-reveal mx-auto max-w-6xl">
          <p className="mb-4 text-lg font-semibold uppercase tracking-[0.45em] text-white">
            Resources
          </p>

          <h1 className="font-serif text-5xl font-bold uppercase leading-[1.05] text-white md:text-6xl">
            Student Resources
          </h1>
      ...
      <section className="px-10 py-16">
        <div className="mx-auto grid max-w-6xl gap-8 md:grid-cols-2">
          {resources.map((resource, index) => (
            <article
              key={resource.title}
              className={`rounded-[28px] border border-white/80 bg-white p-4 text-blue-950 shadow-xl shadow-blue-950/10 transition duration-300 ease-out hover:-translate-y-1.5 hover:border-[#7fffd6] hover:shadow-2xl hover:shadow-blue-950/20 ${
                index % 2 === 0 ? "page-reveal-delay-1" : "page-reveal-delay-2"
              }`}
            >
              <div className="flex h-full flex-col overflow-hidden rounded-[24px] border-4 border-[#008c5a] bg-white">
                <div className="h-64 overflow-hidden border-b-4 border-[#d7eefb] bg-[#e9f8ff] sm:h-72">
                  <img
                    src={resource.image}
                    alt={resource.alt}
                  ...
                  <div className="mt-7 grid gap-3 sm:grid-cols-2">
                    {resource.links.map((link) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="flex min-h-12 items-center justify-center rounded-xl bg-blue-950 px-4 py-3 text-center text-sm font-bold text-white shadow-md shadow-blue-950/20 transition duration-300 hover:-translate-y-1 hover:bg-[#008c5a] hover:shadow-lg hover:shadow-green-500/20"
                      >
                        {link.text}
                      </a>
                    ))}
